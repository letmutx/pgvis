    ...
    function Neo4jD3(_selector, _options) {
      ...
      function appendTextToNode(node) {
        return node.append('text')
          ...
          .attr('font-size', function(d) {
            return icon(d) ? (d.nodeRadius + 'px') : '10px';
          })
          .attr('pointer-events', 'none')
          .attr('text-anchor', 'middle')
          .attr('y', function(d) {
            return icon(d) ? (parseInt(Math.round(d.nodeRadius* 0.32)) + 'px') : '4px';
          })
          .html(function(d) {
            var _icon = icon(d);
      ...
      function init(_selector, _options) {
        ...
        if (!options.minCollision) {
          options.minCollision = d.nodeRadius * 2;
        }
      ...
      function tickRelationshipsOutlines() {
        relationship.each(function(relationship) {
          ...
          outline.attr('d', function(d) {
            ...
            rotatedPointA1 = rotatePoint(center, { x: 0 + (d.nodeRadius + 1) * u.x - n.x, y: 0 + (d.nodeRadius + 1) * u.y - n.y }, angle),
            rotatedPointB1 = rotatePoint(center, { x: textMargin.x - n.x, y: textMargin.y - n.y }, angle),
            rotatedPointC1 = rotatePoint(center, { x: textMargin.x, y: textMargin.y }, angle),
            rotatedPointD1 = rotatePoint(center, { x: 0 + (d.nodeRadius + 1) * u.x, y: 0 + (d.nodeRadius + 1) * u.y }, angle),
            rotatedPointA2 = rotatePoint(center, { x: d.target.x - d.source.x - textMargin.x - n.x, y: d.target.y - d.source.y - textMargin.y - n.y }, angle),
            rotatedPointB2 = rotatePoint(center, { x: d.target.x - d.source.x - (d.nodeRadius + 1) * u.x - n.x - u.x * options.arrowSize, y: d.target.y - d.source.y - (d.nodeRadius + 1) * u.y - n.y - u.y * options.arrowSize }, angle),
            rotatedPointC2 = rotatePoint(center, { x: d.target.x - d.source.x - (d.nodeRadius + 1) * u.x - n.x + (n.x - u.x) * options.arrowSize, y: d.target.y - d.source.y - (d.nodeRadius + 1) * u.y - n.y + (n.y - u.y) * options.arrowSize }, angle),
            rotatedPointD2 = rotatePoint(center, { x: d.target.x - d.source.x - (d.nodeRadius + 1) * u.x, y: d.target.y - d.source.y - (d.nodeRadius + 1) * u.y }, angle),
            rotatedPointE2 = rotatePoint(center, { x: d.target.x - d.source.x - (d.nodeRadius + 1) * u.x + (- n.x - u.x) * options.arrowSize, y: d.target.y - d.source.y - (d.nodeRadius + 1) * u.y + (- n.y - u.y) * options.arrowSize }, angle),
            rotatedPointF2 = rotatePoint(center, { x: d.target.x - d.source.x - (d.nodeRadius + 1) * u.x - u.x * options.arrowSize, y: d.target.y - d.source.y - (d.nodeRadius + 1) * u.y - u.y * options.arrowSize }, angle),
